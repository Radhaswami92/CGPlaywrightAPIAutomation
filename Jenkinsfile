        ...
        stage('Run Tests inside Playwright Container') {
            steps {
                bat '''
                    ...
                    bash -c "python -m pip install --upgrade pip && pip install pytest pytest-bdd pytest-playwright && python -m pytest Learn_Playwright_BDD_Framework/StepDefinitionFiles"
                '''
            }
        }
    }
